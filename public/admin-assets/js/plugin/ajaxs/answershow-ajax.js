function answershow(id) {
    let element = $("#" + id),
        ...
        console.log(element,url,elementValue,id);
    $.ajax({
        url: url,
        type: "GET",
        success: function (response) {
            if (response.answershow) {
                if (response.checked) {
                    changeState(
                        '<i class="fa fa-eye-slash ms-1 align-middle"></i>عدم نمایش پاسخ در نظرات',
                        ["btn-success", "btn-danger"],
                        true
                    );
                    successToast(
                        "پاسخ نظر کاربر با شماره شناسه " +
                        response.id +
                        " با موفقیت در نظرات نمایش داده شد"
                    );
                } else {
                    changeState(
                        '<i class="fa fa-eye ms-1 align-middle"></i> نمایش پاسخ در نظرات',
                    ...
                    successToast(
                        "پاسخ نظر کاربر با شماره شناسه " +
                            response.id +
                            " از حالت نمایش در نظرات خارج شد"
                    );
                }
            } else {
    ...
    function changeState(text, btnStyle, checkProp) {
        element.prop("checked", checkProp);
        element.removeClass(btnStyle[0]);
        element.addClass(btnStyle[1]);
        element.html(text);
    }
