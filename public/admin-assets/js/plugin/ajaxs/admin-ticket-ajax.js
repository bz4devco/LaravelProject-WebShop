        ...
        success: function (response) {
            if (response.set) {
                if (response.checked) {
                    ...
                    element.html('<i class="fa fa-times ms-1 align-middle"></i>حذف');
                    successToast(
                        "ادمین " +
                            response.fullName +
                            " به لیست مسئول تیکت اضافه شد"
                    );
                } else {
                    element.removeClass('bg-danger');
                    element.addClass('bg-success');
                    element.html('<i class="fa fa-check ms-1 align-middle"></i>افزودن');
                    successToast(
                        "ادمین " +
                            response.fullName +
                            " از لیست مسئول تیکت حذف شد"
                    );
                }
            } else {
