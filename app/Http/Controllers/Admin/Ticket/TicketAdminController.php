...
class TicketAdminController extends Controller
{
    ...
    {
        $admins = User::where('user_type', 1)->orderBy('first_name', 'asc')->simplePaginate(15);
    ...
    // {
    //     if($admin->user_type == 1){
    //         dd(!isset($admin->ticketAdmin->tickets));
    //        if(!isset($admin->ticketAdmin->tickets)){
    //             $result = $ticketAdmin->where('user_id', $admin->id)->first() ?
    //             $ticketAdmin->where(['user_id' => $admin->id])->forceDelete() :
    //             $ticketAdmin->create(['user_id' => $admin->id]);
    ...
    //             }
    //         }else{
    //             return response()->json(['set' => true, 'isReference' => true, 'fullName' => $admin->full_name]);
    //         }
    //     }else{
    //         return response()->json(['set' => false]);
    //     }
